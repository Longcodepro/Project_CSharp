...
| `POST /api/auth/logout` | Có | User đang đăng nhập | JWT stateless nên API chỉ xác nhận đăng xuất; client cần xóa token | Nếu muốn vô hiệu hóa token phía server cần thêm blacklist/session schema |
...
| `POST /api/favorites/{mediaId}` | Có | Có, qua `ICurrentUserContext` | User chỉ tạo/cập nhật reaction cho chính mình | Body tùy chọn `{ reaction }`; không gửi body thì mặc định `Like`; response trả reaction đã chọn; check media tồn tại `IsActive = 1`, `IsPublic = 1`, `IsValid = 0`; không tạo duplicate | Không có |
| `DELETE /api/favorites/{mediaId}` | Có | Có | User chỉ xóa reaction của chính mình | Nếu chưa có favorite thì không crash; response trả `data = null` vì không còn reaction | Không có |
| `GET /api/favorites/me` | Có | Có | Chỉ lấy reaction của user hiện tại | Trả danh sách gọn gồm tên media và reaction đã chọn | Không có |
| `GET /api/favorites/status/{mediaId}` | Có | Có | Chỉ check reaction của user hiện tại | Trả reaction cụ thể hoặc `null` nếu chưa thể hiện cảm xúc | Không có |
...
| `PATCH /api/history/{mediaId}/stop` | Có | Có | User chỉ lưu trạng thái dừng cho chính mình | Ghi/cập nhật `StoppedAt`; nếu không truyền body thì dùng thời điểm server hiện tại | Schema hiện chưa có cột lưu số giây phát như `StoppedSeconds` |
| `GET /api/history/{mediaId}/resume` | Có | Có | User chỉ lấy resume info của chính mình | Trả `mediaId`, `title`, `stoppedAt` nếu media có trong lịch sử | Chỉ trả thời điểm dừng, chưa trả vị trí giây phát do schema chưa có |
