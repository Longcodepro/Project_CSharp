...
| 19 | `CollectionLikes` | Lượt thích album/playlist |
...
| `LikedAt` | `datetime2` | NOT NULL | `SYSUTCDATETIME()` |

**Constraints**

- `CONSTRAINT PK__CollectionLikes__3214EC07XXXXXX PRIMARY KEY (Id)`
- `CONSTRAINT FK__CollectionLikes__UserId__YYYYYY FOREIGN KEY (UserId) REFERENCES TuneVault.dbo.Users(Id)`
...
- `Purpose` hiện được backend chấp nhận theo các giá trị `register`, `reset_password`, `change_password`.
...
- `IX_Favorites_User_Target` on `UserId ASC , TargetType ASC , TargetId ASC`

**Ghi chú**

- `Favorites` hiện đã mở rộng cho `Media`, `Album`, `Playlist` thông qua `TargetId` và `TargetType`.
- Cột `MediaItemId` cũ đã được bỏ khỏi schema, dữ liệu media cũ cần đi qua `TargetId`.
- `TargetType` dùng map theo `FavoriteTargetType` ở backend.
...
- `PlayHistory` hiện có `HistoryOrder`, `StoppedAt`; `StoppedAt` là vị trí dừng theo giây trong media, không phải `datetime`. Bảng này chưa có `PlayedAt`.
