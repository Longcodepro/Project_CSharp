...
public interface IPlayHistoryRepository
{
    ...
    /// <summary>
    /// Lưu bản ghi lịch sử mới nhất của người dùng bằng transaction để đồng bộ thứ tự, vị trí dừng và giới hạn tối đa 10 mục.
    /// </summary>
    /// <param name="playHistory">Bản ghi lịch sử đã được validate ở Domain layer.</param>
    /// <param name="isNewRecord">True nếu cần insert mới, false nếu cập nhật bản ghi hiện có.</param>
    /// <param name="ct">Token hủy tác vụ bất đồng bộ.</param>
    Task SaveRecentPlaybackAsync(PlayHistory playHistory, bool isNewRecord, CancellationToken ct = default);
}
