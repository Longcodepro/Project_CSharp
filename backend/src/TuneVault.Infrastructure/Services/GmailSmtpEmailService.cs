...
public class GmailSmtpEmailService : IEmailService
{
    ...
    public async Task SendOtpAsync(string toEmail, string otpCode, string purpose, CancellationToken ct = default)
    {
        ...
        {
            "register" => "TuneVault — Xác nhận đăng ký tài khoản",
            "reset_password" => "TuneVault — Yêu cầu đặt lại mật khẩu",
            "change_password" => "TuneVault — Yêu cầu đổi mật khẩu",
            _ => "TuneVault — Mã xác nhận"
        };

        var actionLabel = purpose switch
        {
            "register" => "hoàn tất đăng ký tài khoản",
          ...
          <tr>
            <td style=""padding:40px 40px 32px;"">
              ...
              <p style=""margin:0 0 24px;color:#aaaaaa;font-size:13px;text-align:center;"">
                ⏱ Mã có hiệu lực trong <strong style=""color:#e0e0e0;"">1 phút 30 giây</strong>
              </p>
